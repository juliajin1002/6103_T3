...
#%%
# Model construction
# Start with full model and do backwards stepwise selection

modelBank = glm(formula = 'y ~ age + marital + education + default + housing + loan + contact + month + day_of_week + duration + campaign + pdays + previous + poutcome + emp_var_rate + cons_price_idx + cons_conf_idx + euribor3m + nr_employed', data=bank_df, family=sm.families.Binomial())
...
modelBank = glm(formula = 'y ~ age + education + default + contact + month + day_of_week + duration + campaign + pdays + poutcome + emp_var_rate + cons_price_idx + cons_conf_idx + euribor3m', data=bank_df, family=sm.families.Binomial())
modelBankFit = modelBank.fit()
print(modelBankFit.summary())
